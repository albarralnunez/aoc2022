...
class Ship:
    ...
    def move_p1(self, instruction: Instruction) -> None:
        new_stack, items_to_move = self._split(
            self.stacks[instruction.move_from - 1].cargo, instruction.move
        )
...
class ShipFactory:
    ...
    instruction_line_re: ClassVar[re.Pattern] = re.compile(
    ...
    def _read_blueprint(self, f) -> list[str]:
        blueprint = []
        for line in f:
            if line == "\n":
                break
    ...
    def read_input(self, f: IO) -> tuple[Ship, Iterator[Instruction]]:
        ship = Ship()
    ...
    def read(self, f: IO) -> tuple[Ship, Iterator[Instruction]]:
        ship = Ship()
        blueprint = self._read_blueprint(f)
        self._blueprint_to_ship(blueprint, ship)
        instructions = self._read_instructions(f)
